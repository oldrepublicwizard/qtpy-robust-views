...
from qtpy_robust_views.itemviews.listview import RobustListView
...
class RobustListWidget(QListWidget, RobustListView):
    ...
    def build_context_menu(self) -> QMenu:
        ...
        self._add_simple_action(
            ...
            lambda: self.sortItems(Qt.SortOrder.AscendingOrder),
        )
        self._add_simple_action(
            list_widget_menu,
            "Sort Items (Descending)",
            lambda: self.sortItems(Qt.SortOrder.DescendingOrder),
        )
        ...
        self._add_simple_action(
            item_menu, "Insert Item", lambda: self.insertItem(self.currentRow(), "Inserted Item")
        )
        self._add_simple_action(
            item_menu, "Remove Current Item", lambda: self.takeItem(self.currentRow())
        )
        self._add_simple_action(item_menu, "Clear All Items", self.clear)
        self._add_simple_action(
            item_menu, "Edit Current Item", lambda: self.editItem(self.currentItem())
        )
        ...
        self._add_menu_action(
            view_menu, "Wrap Items", self.isWrapping, self.setWrapping, "wrapping"
        )
        self._add_menu_action(
            view_menu,
            "Uniform Item Sizes",
        ...
        self._add_menu_action(
            advanced_menu, "Drag Enabled", self.dragEnabled, self.setDragEnabled, "dragEnabled"
        )
        self._add_menu_action(
            advanced_menu, "Drop Enabled", self.acceptDrops, self.setAcceptDrops, "dropEnabled"
        )
